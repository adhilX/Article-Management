...
export default function EditArticlePage() {
  // Pre-fill with dummy active data for the UI
  const [title, setTitle] = useState("Building Microservices with Node.js and Docker");
  const [tags, setTags] = useState("DevOps, Node.js");
  const [content, setContent] = useState("Learn how to establish a robust microservices architecture utilizing containerization to ensure your software is infinitely scalable and easy to deploy across environments.\n\nThe shift from monolithic applications to microservices has empowered development teams to scale rapidly.");
    ...
    <div className="max-w-4xl mx-auto w-full pt-8 pb-32">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 border-b border-white/10 pb-4 gap-4">
        ...
        <div className="flex gap-4 items-center">
          <button className="text-sm font-medium text-rose-500 hover:text-rose-400 transition-colors">
            Discard changes
          </button>
          <button 
            type="button" 
            ...
            className="btn-primary text-sm px-6 py-2 rounded-full shadow-[0_4px_15px_rgba(168,85,247,0.3)] disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none bg-gradient-to-r from-emerald-500 to-teal-500"
          >
            Update Article
          </button>
        </div>
